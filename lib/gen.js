...
/**
 * dom规则生成查找语句
 * @param  {String} rule dom规则
 * @return {String}  查找语句 
 */
function domParser(rule) {
  if (!rule) throw new Error('请输入dom规则');
 ...
 */
function gen(level) {
  let statement = `$("${level[0]['element']}${level[0]['id']? 
...
exports = module.exports = domParser;
